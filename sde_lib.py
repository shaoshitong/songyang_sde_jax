...
from dpm_solver_jax import interpolate_fn
...
class DS_VPSDE(SDE):
    ...
    def get_contiguous_value(self, t, discrete_value_list):
        if isinstance(t, jnp.ndarray):
            t = t.reshape((-1, 1))
        else:
            t = jnp.array([t]).reshape((-1, 1))
        return interpolate_fn(t, self.t_array, discrete_value_list).reshape((-1))
    ...
    def get_lambda(self, t):
        return self.get_contiguous_value(t,self.discrete_lambda)
